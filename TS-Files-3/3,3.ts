...
function componentToLongHex(c: number): string {
  let hex = c.toString(16);
  return hex.length == 1 ? "0" + hex : hex;
}

function componentToShortHex(c: number): string {
  let hex: string = c.toString(16);
  return hex.length != 1 ? (hex = hex[0]) : hex;
}

function rgbToLongHex(r: number, g: number, b: number): string {
  return (
    "#" + componentToLongHex(r) + componentToLongHex(g) + componentToLongHex(b)
  );
}

function rgbToShortHex(r: number, g: number, b): string {
  return (
    "#" +
